...
def generate_tfrecords_V2Core(case_ids, dataset_dir, annotation_dir, save_path):
    num_frames = 0
    with tf.python_io.TFRecordWriter(save_path) as writer:
        for case_id in tqdm(case_ids):
            cur_image_path = os.path.join(dataset_dir, '{}.mhd'.format(case_id))
...
def generate_tfrecords_V2(dataset_dir, annotation_dir, save_dir):
    '''
    生成tfrecords, 不截断，使用原始的像素值
    保存的对象包括所有的具有object 的slice和一部分negative的slice, 每个样本选择negative slice和positive slice一样
    :param dataset_dir:
    :param annotation_dir:
    :return:
...
if __name__ == '__main__':
    ...
    generate_tfrecords_V2(dataset_dir, annotation_dir, save_dir)